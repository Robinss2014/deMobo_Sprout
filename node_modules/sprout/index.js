...
merlin.openMat = function(html) {
	...
	if (!matWindow) {
        // demobo
        // open an mat window with the same settings for the main window
        matWindow = global.window.nwDispatcher.requireNwGui().Window.get(
         window.open(html) //('mat.html')
        );

        // demobo
        // open an mat window with the different settings
        // matWindow = global.window.nwDispatcher.requireNwGui().Window.open(html, {"frame": false,"toolbar": false});
...
// demobo
// add new function closeMat
merlin.closeMat = function() {
    if(matWindow) {
        matWindow.close();
    }
    matWindow=null;
...
merlin.lockCapsenseButtons = function () {
    var payload = {
        commandName: "lockCapsenseButtons",
        lockCommand: "lock"
    };
    return execute(payload);
}

merlin.unlockCapsenseButtons = function () {
    var payload = {
        commandName: "lockCapsenseButtons",
        lockCommand: "unlock"
    };
...
merlin.turnProjectorOn = function () {
    var payload = {
        commandName: "turnProjectorSwitch",
        switchCommand: "on"
    };
    return execute(payload);
}

merlin.turnProjectorOff = function () {
    var payload = {
        commandName: "turnProjectorSwitch",
        switchCommand: "off"
    };
...
merlin.enableTouchMat = function () {
    var payload = {
        commandName: "enableTouchMat",
        switchCommand: "enable"
    };
    return execute(payload);
}

merlin.disableTouchMat = function () {
    var payload = {
        commandName: "enableTouchMat",
        switchCommand: "disable"
    };
    return execute(payload);
}
