...
#r "C:\Program Files\Hewlett-Packard\Sprout\SDK\HP.PC.Presentation.Private.dll"
...
using HP.PC.Presentation.Private;
...
namespace MerlinJSBinding
{
    public class Merlin
    {
        ...
        private static IPcPrivateLink pcPrivateLink;
        private static IPcPrivateLink PcPrivateLink
        {
            get
            {
                pcPrivateLink = (pcPrivateLink ?? HPPCPrivate.CreatePrivateLink());
                return pcPrivateLink;
            }
        }
        ...
        public async Task<object> Execute(dynamic input)
        {
            ...
            try
            {
                switch(commandName)
                {
                    ...
                    case "lockCapsenseButtons":
                        return await LockCapsenseButtons(input);
                    case "turnProjectorSwitch":
                        return await TurnProjectorSwitch(input);
                    case "enableTouchMat":
                        return await EnableTouchMat(input);
					case "disposeMoment":
        ...
        private async Task<object> TurnProjectorSwitch(dynamic input)
        {
            try
            {
                IPcHardware hardware = await PcPrivateLink.AccessHardwareAsync();

                //turn on/off projector
                IPcDevice projectorDevice = hardware.GetDevice(PcDeviceType.MatScreen);
                await projectorDevice.SetPropertyAsync(new PcDeviceProperty(PcDevicePropertyName.IsEnabled, input.switchCommand == "on"));
        ...
        private async Task<object> EnableTouchMat(dynamic input)
        {
            try
            {
                IPcHardware hardware = await PcPrivateLink.AccessHardwareAsync();
                ...
                //turn on/off projector
                IPcDevice projectorDevice = hardware.GetDevice(PcDeviceType.MatTouch);
                await projectorDevice.SetPropertyAsync(new PcDeviceProperty(PcDevicePropertyName.IsEnabled, input.switchCommand == "enable"));
            }
            catch (Exception e)
            {
                throw new Exception(e.Message);
        ...
        private async Task<object> LockCapsenseButtons(dynamic input)
        {
            try 
            { 
                IPcHardware hardware = await PcPrivateLink.AccessHardwareAsync();

		//Turn on/off capsense
		var buttonsDevice = hardware.GetDevice(PcDeviceType.CapSenseButtons);
		await buttonsDevice.SetPropertyAsync(new PcDeviceProperty(PcDevicePropertyName.IsEnabled,input.lockCommand != "lock"));
            }
            catch (Exception e)
            {
                throw new Exception(e.Message);
        ...
        private async Task<object> CreateTrackingHandler(dynamic input)
        {
			...
			for (int i = 0; i < picture.Children.Count(); i++)
			{
				...
				/*TrackingHandlerDataObject trackedObject = new TrackingHandlerDataObject(picture.Children.ElementAt(i).Image,
					outline.Children.ElementAt(i).Contour, picture.Children.ElementAt(i).PhysicalBoundaries.ToScreenCoordinates(mat),
					"Object " + i);
				combolist.Add(trackedObject);*/
			}
			
			
			// Add training images
			/* (var item in DataContext.ItemCollection)
			{
            ...
            /*object[] imageURIs = (object[])input.imageURIs;
            if (imageURIs.Length == 0)
            {
                throw new Exception("no image URI is provided");
            }
			await StartImageTracking(imageURIs);
			*/
        ...
        /*
        private IPcTrackingHandler _imagesTrackHandler = null;

        private async Task<object> StartImageTracking(object[] imageURIs)
        {            
            try
            {
                if (_imagesTrackHandler == null)
                {
                    List<IPcPicture> _trainedImages = new List<IPcPicture>();
                    foreach (var imageURI in imageURIs)
                    {
                        var moment = await PcLink.CaptureMomentAsync(new BitmapImage(new Uri(Convert.ToString(imageURI), UriKind.RelativeOrAbsolute)));
                        var picture = await PcLink.ExtractPictureAsync(moment);
                        _trainedImages.Add(picture);
                    }

                    //Register trained images with platform service and create track handler
                    _imagesTrackHandler = PcLink.CreateTrackingHandler(_trainedImages);

                    _imagesTrackHandler.TrackUpdated += _imagesTrackHandler_TrackUpdated;
                    await _imagesTrackHandler.StartAsync();
                }
                return true;
            }
            catch (Exception e)
        ...
        private async Task<object> StopImageTracking()
        {
            ...
            if (_imagesTrackHandler != null)
            {
                await _imagesTrackHandler.StopAsync();

                _imagesTrackHandler.TrackUpdated -= _imagesTrackHandler_TrackUpdated;
                _imagesTrackHandler.Dispose();
                _imagesTrackHandler = null;
                return true;
            }
            else
            {
